...
emg_input = np.zeros((1, 32, 8))
index = 0
modelg = load_model('/home/josecyc/new_Neurotron/misc/saved_models/model_highloss_jose_2_001.h5')
...
if __name__ == '__main__':
    ...
    def proc_emg(emg, moving, times=[]):
        ...
        queue.append(list(emg))
        
        #global index
        
        #emg_input[0, index, :] = list(emg)
        #index += 1
        #if index == 23 or index == 24:
        if (len(queue) == 24):
            preds = modelg.predict(np.array(queue).reshape(1, 24, 8))
            print('model loaded')
            send_to_godot(preds)
            queue.popleft()
            #np.delete(emg_input, [0, 0, :])
            #index = 23
