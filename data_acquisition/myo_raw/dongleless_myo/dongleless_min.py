...
def write_to_csv(emg)#, num, person):
    with open('emg_data_{}_{}.csv'.format(num, person), 'a') as fd:
...
4) Then it will wait for notifications from the Myo device with a 3s timeout, if
			...
			while not p:
				...
				for mac in x:
					...
					try:
						...
						log.info("Not actually a Myo at MAC:{}".format(mac))
						#log.info("Found Myo at MAC: %s" % mac)
			btle.DefaultDelegate( MyoDelegate(modes, p) )
